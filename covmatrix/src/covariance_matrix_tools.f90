...
SUBROUTINE covariance_strenght(ensemble,covst_h,corrst_h,corrstdist_h,nx,ny,nz,nbv,delta,undefmask,undefbin)
IMPLICIT NONE
INTEGER, INTENT(IN) :: nx,ny,nz,nbv,delta
REAL(r_sngl), INTENT(IN) :: ensemble(nx,ny,nz,nbv)  , undefbin
LOGICAL , INTENT(IN)     :: undefmask(nx,ny,nz) 
REAL(r_sngl), INTENT(OUT) :: covst_h(nx,ny,nz),corrst(nx,ny,nz),corrstdist(nx,ny,nz) !Covariance and correlation strhenght index.
